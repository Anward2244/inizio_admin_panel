        ...
        <div className="fixed inset-0 z-100 flex items-center justify-center p-4 lg:p-8">
          ...
          <div className="relative bg-slate-900 border border-white/10 shadow-2xl rounded-3xl mx-4 w-full max-w-2xl overflow-hidden flex flex-col max-h-[80vh] animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="pt-3 pb-4 px-5 border-t border-white/10 flex justify-end gap-3 bg-slate-800/30 shrink-0">
              <button onClick={() => handleReject(selectedUser._id)} disabled={isActionLoading} className="px-6 py-2.5 bg-transparent border border-red-500/30 text-red-400 font-bold rounded-xl hover:bg-red-500/10 transition-colors disabled:opacity-50">
                Reject KYC
              </button>
              <button onClick={() => handleApprove(selectedUser._id)} disabled={isActionLoading} className="flex items-center px-6 py-2.5 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 shadow-lg shadow-emerald-500/30 transition-all disabled:opacity-50">
